...
{
    public partial class AudittedVouchers: Bosco.Report.Base.ReportHeaderBase
    {
        #region Constructor
        public AudittedVouchers()
        {
            InitializeComponent();
        ...
        private void SetReportProperty()
        {
            ...
            {
                DataTable dtAudotedVouchers = resultArgs.DataSource.Table;
                GrpHeaderTrackType.Visible = grpVoucherHeader.Visible = grpVoucherFooter.Visible = false;
                Detail.Visible = GrpHeaderTrackType.Visible;
                if (dtAudotedVouchers != null && dtAudotedVouchers.Rows.Count != 0)
                {
                    dtAudotedVouchers.TableName = "AuditVoucherLog";
                    ...
                    {
                        dtAudotedVouchers.DefaultView.RowFilter = auditaction;
                        dtAudotedVouchers = dtAudotedVouchers.DefaultView.ToTable();
                    }
                    //-----------------------------------------------------------------------
                    ...
                    {
                        dtAudotedVouchers.DefaultView.RowFilter = userfitler;
                        dtAudotedVouchers = dtAudotedVouchers.DefaultView.ToTable();
                    }
                    //----------------------------------------------------------------------
                    ...
                    string filtercondition = this.AttachAmountFilter(dtAudotedVouchers.DefaultView, filterfields);
                    ...
                    this.DataSource = dtAudotedVouchers;
                    this.DataMember = dtAudotedVouchers.TableName;
                    GrpHeaderTrackType.Visible = grpVoucherHeader.Visible = grpVoucherFooter.Visible = (dtAudotedVouchers.Rows.Count > 0);
                    Detail.Visible = GrpHeaderTrackType.Visible;
                }
                else
                {
        ...
        private ResultArgs GetReportSource()
        {
            try
            {
                string AuditLog = this.GetAuditReportSQL(SQL.ReportSQLCommand.AuditReports.AuditedVouchers);
        ...
        private void grpVoucherHeader_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            grpVoucherFooter.Visible =  xrTblAuditHistory.Visible = false;
            Detail.Visible = false;
            if (GetCurrentColumnValue(reportSetting1.AuditLog.TRACK_VOUCHER_IDColumn.ColumnName) != null)
            {
                Int32 trackvid = this.UtilityMember.NumberSet.ToInteger(GetCurrentColumnValue(reportSetting1.AuditLog.TRACK_VOUCHER_IDColumn.ColumnName).ToString());
                if (trackvid > 0)
                {
                    grpVoucherFooter.Visible = xrTblAuditHistory.Visible = true;
                    Detail.Visible = true;
                }
                else
                {
                    Detail.Visible = grpVoucherFooter.Visible = xrTblAuditHistory.Visible = false;
                    Detail.Visible = false;
                }
        ...
        private void xrTblHeaderAuditHistory_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
           
        }

        private void xrRowAuditHistoryHeader_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            if (GetCurrentColumnValue(reportSetting1.AuditLog.TRACK_VOUCHER_IDColumn.ColumnName) != null)
            {
                XRTable tblAuditHeader = sender as XRTable;
                Int32 trackvid = this.UtilityMember.NumberSet.ToInteger(GetCurrentColumnValue(reportSetting1.AuditLog.TRACK_VOUCHER_IDColumn.ColumnName).ToString());
                if (trackvid > 0)
                {
                    e.Cancel= false;
