...
{
    public partial class StockSales : Bosco.Report.Base.ReportHeaderBase
    {
         #region Declaration
        ...
        private void SetStockSalesReport()
        {
            if (this.ReportProperties.DateFrom != string.Empty || this.ReportProperties.DateTo != string.Empty && this.ReportProperties.Project != "0")
            {
                ...
                DataTable dtStockSales = BindReportSource();
                if (dtStockSales != null)
                {
                    this.DataSource = dtStockSales;
                    this.DataMember = dtStockSales.TableName;
                }
        ...
        {
            string stockSummary = this.GetReportTDS(SQL.ReportSQLCommand.Stock.StockSales);
