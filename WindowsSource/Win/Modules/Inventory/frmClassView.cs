...
{
    public partial class frmClassView : frmBase
    {
       ...
       // AssetStockProduct.IGroup Igroup;
        ...
        #region Costurctor
        public frmClassView(FinanceModule mod)
        {
            module = mod;
        ...
        {
            ShowAssetGroupDetails(AssetClassId, FormMode.Add);
        }
            ...
            if (this.isEditable)
            {
                if (ucAssetGroups.VisibleEditButton == BarItemVisibility.Always)
                {
                    if (trlGroup.Nodes.Count > 0)
                    {
                        if (AssetClassId > 0)
                        {
                            ShowAssetGroupDetails(AssetClassId, FormMode.Edit);
                        }
                        else
                        {
        ...
        private void ShowAssetClassDetails(int ClassId, FormMode frmMode)
        {
            try
            {
                frmGroupAdd assetGroupItem = new frmGroupAdd(ClassId, module, frmMode);
