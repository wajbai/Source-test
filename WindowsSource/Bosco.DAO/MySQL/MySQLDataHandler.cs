...
namespace Bosco.DAO.MySQL
{
    public class MySQLDataHandler : IDatabase
    {
        ...
        public ExecutionMode TransExecutionMode
        {
            set
            {
                executionMode = value;
        ...
        public ResultArgs Execute(DataManager dataManager, SQLType sqlType, bool DontShowErrorMessage = false)
        {
            return Execute(dataManager, "", sqlType);
        }

        public ResultArgs Execute(DataManager dataManager, string SQLStatement, SQLType sqlType, bool DontShowErrorMessage = false)
        {
            ResultArgs result = new ResultArgs();
            ...
            {
                //Update Exception
                ParseException(result, e, DontShowErrorMessage);
                ...
                if (dataParameters != null)
                {
                    foreach (IDataArguments itemValue in dataParameters)
                    {
                        ...
                        if (param != string.Empty)
                        {
                            ...
                            {
                                itemValue.FieldValue = DBNull.Value;
                            }
        ...
        private string ReplaceParameterValue(string query, string param, object paramValue, SQLType sqlType, DataType fieldType)
        {
            ...
            if (sqlType == SQLType.SQLStatic)
            {
                ...
                else
                {
                    ...
                    //Modified by salamon
                    //If the culture is of some other country, The culture is removed.
                    string ParemValue = Convert.ToString(paramValue, CultureInfo.InvariantCulture);
                    sql = query.Replace(param, ParemValue);
                }
            }
        ...
        private void ParseException(ResultArgs result, Exception e, bool DontShowErrorMessage = false)
        {
            ...
            int posStartValue = 0;
            int posEndValue = 0;
            string avilableValue = string.Empty;

            //Validation Message for Adding/Updating duplicate value
            posStart = errorMessage.ToLower().IndexOf("duplicate entry");

            //Getting actual duplicated value 
            posStartValue = posStart + "duplicate entry".Length;
            posEndValue = errorMessage.ToLower().IndexOf("for");

            if (posEndValue >= posStartValue)
            {
                avilableValue = message.Substring(posStartValue, posEndValue - posStartValue);
            }
            ...
            if (posStart >= 0)
            {
                ...
                if (posEnd >= posStart)
                {
                    message = "The Record is Available" + avilableValue; //errorMessage.Substring(posStart, (posEnd - posStart + 1)) + " is available";
                    goto ENDLINE;
                }
            }
            ...
            if (posStart >= 0)
            {
                message = MessageRender.GetMessage(MessageCatalog.Common.CANNOT_DELETE); //"Cannot Delete. This Record has association";
            }
        ...
        ENDLINE:
            ...
            {
                ((ExceptionHandler)result.Exception).Add(e, message, true);


                AcMELog.WriteLog(e.Message);
            }
            else
            {
