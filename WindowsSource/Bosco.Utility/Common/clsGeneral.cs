...
namespace Bosco.Utility.Common
{
    public class clsGeneral
    {
        ...
        public static string RPTPROJECTID = string.Empty;
        ...
        public enum LeavingReason
        {
            ...
            Resigned = 2,
            [Description("Family Committments")]
            FamilyCommittments = 3,
            [Description("Relocation")]
            Relocation = 4,
            [Description("Higher Education")]
            HigherEducation = 5,
            [Description("Contract is Over")]
            ContractisOver = 6,
            [Description("Better Opportunity")]
            BetterOpportunity = 7,
            [Description("Not Satisfied with Salary")]
            NotSatisfiedwithSalary = 8,
        ...
        /// <summary>
        /// To check wheather payroll exists or not
        /// </summary>
        /// <returns></returns>
        public static bool checkPayrollexists()
        {
            bool isExists = true;
            if (PAYROLL_ID <= 0)
            {
                XtraMessageBox.Show("Payroll does not Exists, Create/Open Payroll", "Payroll", MessageBoxButtons.OK, MessageBoxIcon.Information); ;
                isExists = false;
            }
            return isExists;
        }
        ...
        /// <summary>
        /// This method is used to check Acmeerp updater is running whenever acmeerp is started
        /// </summary>
        /// <returns></returns>
        public static bool IsAcmeerpUpdaterRunning()
        {
            bool Rtn = true;
            try
            {
            ...
            catch (Exception err)
            {
                ...
                AcMELog.WriteLog("Error in IsAcmeerpUpdaterRunning " + err.Message);
            }
            return Rtn;
        }

        public static string ReadPasswordProtectedExcel(string ExcelFilePath, string Password)
        {
            String TempExcelFilePath = string.Empty;

            Microsoft.Office.Interop.Excel.Application excelapp = new Microsoft.Office.Interop.Excel.Application();
            excelapp.Visible = false;
            string fileextention = Path.GetExtension(ExcelFilePath);
            string AcmeerpInstalledPath = AppDomain.CurrentDomain.BaseDirectory;
            string AcmeerpTmpPath = Path.Combine(AcmeerpInstalledPath + @"temp\");
            string AcmeerpImportBudgetFile = Path.Combine(AcmeerpTmpPath, "tmpbudget." + fileextention);
            if (!Directory.Exists(AcmeerpTmpPath))
            {
                Directory.CreateDirectory(AcmeerpTmpPath);
            }
            if (File.Exists(AcmeerpImportBudgetFile))
            {
                File.Delete(AcmeerpImportBudgetFile);
            }

            try
            {

                //Microsoft.Office.Interop.Excel.Workbooks wb = excelapp.Workbooks;
                //Microsoft.Office.Interop.Excel.Workbook newWorkbook = wb.Open(ExcelFilePath, false, false, false, "na");
                ...
                newWorkbook.SaveAs(AcmeerpImportBudgetFile, Type.Missing, "");

                //newWorkbook.SaveAs(@"D:\test.xlsx", Microsoft.Office.Interop.Excel.XlFileFormat.xlWorkbookDefault, "", "",
                ...
                if (File.Exists(AcmeerpImportBudgetFile))
                {
                    TempExcelFilePath = AcmeerpImportBudgetFile;
                }
            }
            catch (Exception err)
            {
        ...
        public static DataTable ReadExcelFileInOLEDB(string _ExcelFilePath)
        {
        ...
        public static ResultArgs ExcelToDataTable(string filepath, string sheetname)
        {
            ResultArgs resultargs = new ResultArgs();
            //string FilePath = @"C:\Users\Alwar\Downloads\BudgetView.xls";
            string sConnectionString = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + filepath + ";Extended Properties=\"Excel 12.0;HDR=Yes;IMEX=2\"";
            try
            {
                DataTable dt = new DataTable(sheetname);
                using (OleDbConnection m_connection = new OleDbConnection(sConnectionString))
                {
        ...
        try
        {
            string filePath = @"C:\Users\Alwar\Desktop\Acme.erp\Budget CRs\CMF_Budget\Ampati - NE Delegation - Budget Proposal - 2022.xlsx";
            ...
            string frmCell = string.Empty;
            string toCell = string.Empty;

            for (int iRow = 2; iRow <= workSheet.UsedRange.Rows.Count; iRow++)
            {
                Microsoft.Office.Interop.Excel.Range Firstcell = workSheet.UsedRange.Cells[iRow, 1] as Microsoft.Office.Interop.Excel.Range;
                Microsoft.Office.Interop.Excel.Range Approvedcell = workSheet.UsedRange.Cells[iRow, 10] as Microsoft.Office.Interop.Excel.Range;
                
                if (Firstcell.Value!=null && Approvedcell.Value != null)
                {
                    //workSheet.Cells[iRow, 10].

                    string firstcellvalue = Firstcell.Value.ToString();
                    string approvedcellvalue = Approvedcell.Value.ToString();
                    //if (approvedcellvalue.ToUpper() == "APPROVED INCOME")
                    Approvedcell.Value = "=1+2";
                }
                
                /*for (int iColumn = 1; iColumn < lastRowNumber; iColumn++)
                
                {
                    Microsoft.Office.Interop.Excel.Range cell = workSheet.UsedRange.Cells[iRow, iColumn] as Microsoft.Office.Interop.Excel.Range;
