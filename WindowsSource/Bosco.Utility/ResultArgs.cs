...
namespace Bosco.Utility
{
    public class ResultArgs : IDisposable
    {
        ...
        {
            get { return this.exception.IsDBError; }
            set { this.exception.IsDBError = value; }
        }
        ...
        public Exception Exception
        {
            ...
            set
            {
                ...
                //Modified by alwar on 10/12/2015, if DB error occurs, call updater automatically
                if (this.exception.IsDBError)
                {
                    ...
                    if (XtraMessageBox.Show("Acme.erp is not upto date. Do you want to update the latest version now?",
                        "Acme.erp", MessageBoxButtons.YesNo, MessageBoxIcon.Error) == DialogResult.Yes)
                    {
                        Form frmupdater = this.exception.GetUpdaterForm();
                        if (frmupdater != null)
                        {
                            isShowExceptionMessage = false;
                            frmupdater.ShowDialog();

                            if (frmupdater.DialogResult == DialogResult.OK)
                            {
                                Utility.ConfigSetting.SettingProperty.Is_Application_Logout = true;
                                Application.Exit();
                            }
                        }
                    }
                ...
                {
                    //21/03/2017, If could not establish database connection, show proper message insed of (Unable to connect to any of the specified MySQL hosts)
                    if (value.Message.Contains("Unable to connect to any of the specified MySQL hosts"))
                    {
                        MessageRender.ShowMessage("Could not connect to Database Server. Contact Acme.erp Support Team", true);
                    }
                    else
                    {
